...
COMMENT ON COLUMN tosalpee.documents.current_stage       IS '4-stage 파이프라인 현재 위치 (AUTHORITY_DOC_ISSUED | TRANSLATOR_DOC_RECEIVED | TRANSLATOR_DOC_NOTARIZED | APOSTILLE_DOC_ISSUED).';
...
COMMENT ON COLUMN tosalpee.document_stages.stage          IS '단계 (AUTHORITY_DOC_ISSUED | TRANSLATOR_DOC_RECEIVED | TRANSLATOR_DOC_NOTARIZED | APOSTILLE_DOC_ISSUED | INSTITUTION_DOC_SUBMIT). INSTITUTION_DOC_SUBMIT 는 파이프라인 종료 후 기관 제출 산출물 이벤트로만 기록 — documents.current_stage 에는 들어가지 않음.';
