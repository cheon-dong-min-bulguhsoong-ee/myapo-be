...
describe('UserService', () => {
  ...
  describe('register', () => {
    ...
    it('새로운 유저를 성공적으로 등록한다', async () => {
      ...
      repository.create.mockResolvedValue(
        new User(BigInt(1), input.email, input.name, input.nationality, new Date(), new Date(), false),
      );
  ...
  describe('getProfile', () => {
    it('유저 프로필을 성공적으로 조회한다', async () => {
      const user = new User(BigInt(1), 'a@a.com', 'N', 'KR', new Date(), new Date(), false);
      const wallet = new UserWallet(
        BigInt(1),
        BigInt(1),
        VerifierType.GOOGLE,
    ...
    it('삭제된 유저 조회 시 DomainError를 던진다', async () => {
      const user = new User(BigInt(1), 'a@a.com', 'N', 'KR', new Date(), new Date(), true);
      repository.findById.mockResolvedValue(user);
