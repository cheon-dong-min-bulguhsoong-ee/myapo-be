...
import { DisputeResult, DisputeSummaryResult, TimelineEntryResult } from '../dto/dispute.result';
...
export class DisputeService {
  ...
  async createDispute(input: {
    ...
    await this.logTimeline(id, DisputeStatus.RECEIVED, 'Dispute created.', null, false);
  ...
  async getDispute(id: string, requesterId?: bigint): Promise<DisputeResult> {
    ...
    if (!dispute) {
      throw new DomainError(ErrorCode.Common.NOT_FOUND, { message: 'Dispute not found.' });
    }
  ...
  async assignOperator(disputeId: string, adminId: bigint): Promise<DisputeResult> {
    ...
    if (workloads.length === 0) {
      throw new DomainError(ErrorCode.Common.BAD_REQUEST, { message: 'No operators available.' });
    }
    ...
    await this.logTimeline(disputeId, DisputeStatus.ASSIGNED, `Operator assigned (ID: ${selected.operatorId})`, adminId, true);
  ...
  async changeStatus(input: {
    ...
    dispute.changeStatus(input.newStatus, input.operatorId, input.note, input.isInternal);
    await this.disputeRepository.update(dispute);
    await this.logTimeline(input.id, input.newStatus, input.note, input.operatorId, input.isInternal);
  ...
  private async logTimeline(disputeId: string, status: DisputeStatus, note: string | null, operatorId: bigint | null, isInternal: boolean): Promise<void> {
  ...
  private mapToResult(dispute: Dispute, hideInternal: boolean = false): DisputeResult {
    ...
    return new DisputeResult(
      ...
      dispute.slaDeadline,
      dispute.isSlaPaused,
      filteredTimeline.map(t => new TimelineEntryResult(
        t.id,
        t.status,
        t.note,
