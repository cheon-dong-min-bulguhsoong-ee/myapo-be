...
export class DocumentController {
  ...
  @UseInterceptors(
    FileInterceptor("file", {
      limits: { fileSize: DocumentFileService.MAX_FILE_BYTES },
    }),
  )
  @UploadDocumentFileSwaggerApi()
  async uploadFile(
    ...
    const response = await this.documentFacade.uploadFile(
      file,
      request,
      userId,
    );
  ...
  async uploadEncryptedPdf(
    ...
    const response = await this.documentFacade.uploadEncryptedPdf(
      file,
      request,
      userId,
    );
