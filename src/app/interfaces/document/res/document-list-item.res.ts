...
export class DocumentListItemRes {
    ...
    @ApiProperty({
        description: '요청한 사용자의 내부 회원 식별자 (User.id 문자열). 와이어프레임 컬럼 "회원번호".',
        example: '1',
    })
    ...
    @ApiProperty({description: '문서 카탈로그 표시명 (한국어).', example: '납세증명서'})
    readonly documentTypeName!: string;

    @ApiProperty({description: '발급기관 국가 코드 (ISO 3166-1 alpha-2).', example: 'KR'})
    readonly countryCode!: string;

    @ApiProperty({description: '발급 요청 시각 (ISO 8601, UTC).', example: '2026-05-06T03:00:00.000Z'})
    readonly requestedAt!: string;

    @ApiProperty({enum: DocumentStatus, description: '문서 상태.', example: DocumentStatus.AWAITING_APPROVAL})
    readonly status!: DocumentStatus;

    @ApiProperty({enum: DocumentStage, description: '현재 stage.', example: DocumentStage.AUTHORITY_ISSUED})
