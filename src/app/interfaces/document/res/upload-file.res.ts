...
export class UploadFileRes {
  @ApiProperty({
    description:
      "버킷 내 객체 키. 향후 메타데이터 조회·삭제 API 식별자로 사용 가능.",
  ...
  @ApiProperty({
    description:
      "업로드 시 클라이언트가 보낸 원본 파일 이름(경로 구분자/제어 문자 sanitize 적용).",
  ...
  @ApiProperty({
    description:
      "저장된 객체의 Content-Type. 암호화 PDF 는 `application/pdf` 로 고정.",
  ...
  @ApiProperty({
    description:
      "저장된 객체 크기(byte). 암호화 PDF 는 원본보다 약간 커질 수 있다.",
    example: 38291,
  })
  ...
  @ApiProperty({
    ...
    example:
      "/api/v1/documents/9f2b1a3c-4d5e-6f7a-8b9c-0d1e2f3a4b5c/files/DOCUMENT_ARRIVED",
  })
  readonly downloadUri: string;

  @ApiProperty({
    description:
      "true 면 PDF open-password 가 걸려있어 다운로드 후 비밀번호 입력이 필요.",
    example: false,
  })
