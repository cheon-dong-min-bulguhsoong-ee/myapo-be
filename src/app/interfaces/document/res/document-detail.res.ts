...
export class DocumentStageDetailRes {
    ...
    @ApiProperty({
        ...
        nullable: true,
        description: '단계 진행 상태. null = 아직 시작 안 됨 (DocumentStage 이벤트 행 없음).',
    })
    readonly status!: DocumentStageStatus | null;

    @ApiProperty({type: String, nullable: true, description: '단계 시작 시각 (ISO 8601, UTC).'})
    readonly startedAt!: string | null;

    @ApiProperty({type: String, nullable: true, description: '단계 완료 시각 (ISO 8601, UTC).'})
...
export class DocumentApprovalDetailRes {
    @ApiProperty({enum: DocumentStage, description: '이 승인이 통과시킨 다음 stage.'})
    readonly stage!: DocumentStage;

    @ApiProperty({description: 'XRPL 트랜잭션 해시.', example: 'A5111111111111111111111111111111111111111111111111111111111111AA'})
    readonly xrplTxHash!: string;

    @ApiProperty({description: '서명 컨펌 시각 (ISO 8601, UTC).', example: '2026-05-06T03:00:00.000Z'})
...
export class DocumentDetailRes {
    @ApiProperty({description: '발급 문서 외부 코드.', example: '9f2b1a3c-4d5e-6f7a-8b9c-0d1e2f3a4b5c'})
    readonly documentCode!: string;

    @ApiProperty({description: '내부 회원 식별자 (User.id 문자열).', example: '1'})
    ...
    @ApiProperty({description: '문서 카탈로그 코드.', example: 'KR-NTS-TAX-PAYMENT'})
    ...
    @ApiProperty({description: '발급 요청 시각 (ISO 8601, UTC).', example: '2026-05-06T03:00:00.000Z'})
    readonly requestedAt!: string;

    @ApiProperty({type: String, nullable: true, description: '발급 완료 시각 (WALLET_STORED 도달 시점).'})
