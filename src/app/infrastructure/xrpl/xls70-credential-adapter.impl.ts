...
export class Xls70CredentialAdapterImpl extends XrplCredentialAdapter {
  ...
  buildCredentialDeleteTransaction(
    ...
    if (input.subjectAddress === null && input.issuerAddress === null) {
      throw new DomainError(
        ErrorCode.Credential.XRPL_CREDENTIAL_DELETE_TARGET_REQUIRED,
        {
    ...
    const subject = this.resolveOptionalAccountAddress(
      input.subjectAddress,
      "subjectAddress",
    );
    const issuer = this.resolveOptionalAccountAddress(
      input.issuerAddress,
      "issuerAddress",
    );
  ...
  async submitCredentialCreate(
    ...
    return this.withObjectSnapshot(
      evidence,
      input.subjectAddress,
      input.issuerAddress,
  ...
  async getCredentialObjects(
    ...
    try {
      ...
      return response.result.account_objects
        .map((object) =>
          this.toCredentialObjectResult(object as CredentialObject),
        )
  ...
  private async submitTransaction(
    ...
    try {
      ...
      return this.toTransactionEvidence(
        response,
        transactionKind,
        transaction.Account,
        issuer,
  ...
  private async withObjectSnapshot(
    ...
    return new XrplCredentialTransactionEvidenceResult(
      ...
      object === null
        ...
        : this.objectResultToSnapshot(object),
    );
  }
  ...
  private toTransactionEvidence(
    ...
    const meta = result.meta as
      ...
      | undefined;
    const engineResult =
      typeof meta === "object" && meta.TransactionResult !== undefined
        ? meta.TransactionResult
        : result.engine_result;
    const ledgerIndex =
      typeof result.ledger_index === "number"
        ? BigInt(result.ledger_index)
        : null;
  ...
  private toCredentialObjectResult(
    ...
    return new XrplCredentialObjectResult(
      ...
      object.PreviousTxnLgrSeq === undefined
        ? null
        : BigInt(object.PreviousTxnLgrSeq),
    );
  }

  private objectResultToSnapshot(
  ...
  private matchesCredentialObject(
    ...
    return (
      (input.issuerAddress === null || object.issuer === input.issuerAddress) &&
      (input.subjectAddress === null ||
        object.subject === input.subjectAddress) &&
      (input.credentialTypeHex === null ||
        object.credentialType === input.credentialTypeHex.toUpperCase())
    );
  }
  ...
  private resolveOptionalAccountAddress(
    address: string | null,
    fieldName: string,
  ): string | undefined {
    if (address === null) {
  ...
  private assertCredentialTypeHex(credentialTypeHex: string): void {
    ...
    if (
      credentialTypeHex.length % 2 !== 0 ||
      !/^[0-9a-fA-F]+$/.test(credentialTypeHex)
    ) {
      throw new DomainError(ErrorCode.Credential.XRPL_CREDENTIAL_TYPE_INVALID, {
        reason: "not_hex",
      });
  ...
  private assertUInt32(value: number, fieldName: string): void {
    if (!Number.isInteger(value) || value < 0 || value > UINT32_MAX) {
      throw new DomainError(ErrorCode.Credential.XRPL_UINT32_INVALID, {
        fieldName,
        value,
      });
    }
