...
export class CredentialFacade {
  ...
  async prepareAcceptTestnetCredential(userId: bigint, credentialId: string): Promise<XrplCredentialTransactionRes> {
    const result = await this.credentialService.prepareAcceptTestnetCredential(userId, credentialId);
  ...
  async prepareDeleteTestnetCredential(
    ...
    const result = await this.credentialService.prepareDeleteTestnetCredential(userId, credentialId, req.submitterRole);
    return XrplCredentialTransactionRes.from(result);
  }
