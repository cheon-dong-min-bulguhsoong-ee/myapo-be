...
- **Role**: 사용자의 시스템 권한 등급. Payload에 포함되어 인가(Authorization)에 사용됨.
- **Payload**: JWT 내부에 저장된 사용자 데이터 (Claims). `userId`, `email`, `role` 등을 포함.- **Sub (Subject)**: 토큰의 주체로, MyApo 시스템의 `User.id` (BigInt string).
